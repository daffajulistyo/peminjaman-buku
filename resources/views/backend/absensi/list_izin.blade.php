            ...
            <div class="col-md-12">
                <div class="card card-info">
                    <div class="card-header info">
                        <h3 class="card-title">Input Presensi Pegawai Izin</h3>
                    </div>
                    <form method="POST" action="{{ route('simpan-izin') }}">
                        @csrf
                        <div class="card-body mt-3">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <textarea class="form-control" name="keterangan" rows="5" placeholder="Keterangan Izin" required></textarea>
                                </div>
                            </div>
                            <div class="form-group row">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-info">
                @if (Auth::user()->role == 1)
                    <div class="card-header info">
                        <h3 class="card-title">Daftar Presensi Pegawai Izin</h3>
                    </div>
                @endif
                <!-- /.card-header -->
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{ route('izin.index') }}" method="GET" class="mb-2">
                    ...
                    <table id="exampleTable" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($izin as $key => $i)
                                <tr>
                                    <td>{{ $izin->firstItem() + $key }}</td>
                                    <td>{{ $i->user->name }}</td>
                                    <td>{{ \Carbon\Carbon::parse($i->tanggal)->locale('id_ID')->translatedFormat('j F Y') }}
                                    </td>
                                    <td>{{ $i->keterangan }}
                                    </td>
                                    <td>
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            @if (Auth::user()->role == 1)
                                                <a href="{{ route('izin.edit', ['id' => $i->id]) }}"
                                                    class="btn btn-sm btn-info" data-toggle="modal"
                                                    data-target="#editIzinModal{{ $i->id }}">
                                                    <i class="fas fa-edit"></i> <!-- Ganti dengan ikon Edit -->
                                                </a>
                                            @endif

                                            <a href="{{ route('izin.delete', ['id' => $i->id]) }}"
                                ...
                                <div class="modal fade" id="editIzinModal{{ $i->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Edit Presensi izin</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form role="form" action="{{ route('izin.update', ['id' => $i->id]) }}"
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="date" name="tanggal" value="{{ $i->tanggal }}"
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <textarea class="form-control" name="keterangan" rows="5">{{ $i->keterangan }}</textarea>
                                                        @error('keterangan')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                                            </span>
